...
describe("fan preferences and My Matches", () => {
  ...
  it("builds My Matches from either followed participant", () => {
    ...
    expect(filterMatches(matches, "mine", ["Spain"]).map((match) => match.fixture.fixtureId)).toEqual([18257739, 101, 18198205]);
    expect(filterMatches(matches, "finished", [])).toHaveLength(7);
